...
- **Guido van Rossum** (creator of Python, former BDFL) — reviewed the AAD-50 implementation and responded: *"Very cool."* AAD-50 is implemented in Python; van Rossum's review represents direct acknowledgement from the language's creator of the implementation quality.
...
- **RFC #3** — opened June 25, 2026 on gtrant/eraser (Windows file-level secure deletion): https://github.com/gtrant/eraser/issues/3
...
**June 26, 2026 — nvme-cli v3.0-b.1 RELEASED:** PR #3438 is explicitly listed in the official nvme-cli v3.0-b.1 release changelog. The `--wait` and `--repeat` flags are now shipping in a numbered nvme-cli release.
...
**NDAS bit (No Deallocate After Sanitize).** CDW11 bit 9 of the NVMe Sanitize command controls whether the drive deallocates blocks immediately or lazily. When NDAS = 0 (default), the drive may defer physical deallocation — meaning a read of a sanitized LBA could potentially return original data if the firmware implements lazy deallocation. When NDAS = 1, the drive guarantees physical deallocation. AAD-50 v1.1 does not currently set or verify the NDAS bit explicitly. Future versions will set NDAS = 1 in CDW11 to enforce physical deallocation. *(Gap identified by fzabkar, Top 1% Reddit commenter, June 2026.)*
